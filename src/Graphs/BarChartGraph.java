...
public class BarChartGraph
{
	...
	public BarChart checkCompare(BarChart chart, ArrayList<Integer> array, Device device)
	{
		...
		{
			XYChart.Series readingsTwo = new XYChart.Series(); // create second series of results
			...
			while(i < array.size())
			{
				...
				int random2 = (int)(20 * Math.random() + 15); // get temperature etc here
				readings+= 50; // get recorded time here
				readingsOne.getData().add(new XYChart.Data(Integer.toString((int) readings), lastReadings)); // results 1
				readingsTwo.getData().add(new XYChart.Data(Integer.toString((int) readings), random2)); // results 2
